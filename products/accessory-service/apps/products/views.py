...
from .models import Brand, Category, Product, Inventory, Review
from .serializers import (BrandSerializer, CategorySerializer, ProductListSerializer, ProductDetailSerializer, ProductWriteSerializer, ReviewSerializer, InventorySerializer)
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def reserve(self, request, pk=None):
        ...
        if inventory.available_qty < quantity:
            return Response({'error': f'Insufficient stock. Available: {inventory.available_qty}'}, status=status.HTTP_400_BAD_REQUEST)
